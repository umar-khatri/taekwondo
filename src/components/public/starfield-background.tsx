...
export function StarfieldBackground({
  ...
  useEffect(() => {
    ...
    const maxDepth = 1500
    const currentStarColor = isLight ? "#444444" : "#ffffff"
    const bgFill = isLight ? "#f1f5f9" : "#0a0a0f" // slate-100 for light mode bg
    const fadeFill = isLight ? "rgba(241, 245, 249, 0.3)" : "rgba(10, 10, 15, 0.2)"
    ...
    const animate = () => {
      ...
      for (const star of stars) {
        ...
        const size = Math.max(0.5, (1 - star.z / maxDepth) * (isLight ? 2 : 3))

        // Opacity based on depth (closer = brighter)
        let opacity = (1 - star.z / maxDepth) * 0.9 + 0.1
        if (isLight) opacity *= 0.7 // Softer stars in light mode
  ...
  }, [count, speed, isLight, twinkle])

  return (
    <div
      ref={containerRef}
      className={cn(
        "fixed inset-0 -z-20 overflow-hidden transition-colors duration-500",
        isLight ? "bg-slate-100" : "bg-[#0a0a0f]",
        className
      )}
    >
      ...
      {/* Subtle nebula glow */}
      <div
        className="pointer-events-none absolute inset-0 opacity-30 dark:opacity-40 transition-opacity"
        style={{
          background: isLight
            ? "radial-gradient(ellipse at 30% 40%, rgba(200, 210, 240, 0.4) 0%, transparent 50%), radial-gradient(ellipse at 70% 60%, rgba(240, 200, 210, 0.3) 0%, transparent 50%)"
      ...
      <div
        ...
        style={{
          background: isLight
            ? "radial-gradient(ellipse at center, transparent 0%, transparent 40%, rgba(241, 245, 249, 0.8) 100%)"
